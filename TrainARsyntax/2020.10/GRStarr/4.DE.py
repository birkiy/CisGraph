...
res1hD = pd.read_table("/groups/lackgrp/ll_members/berkay/STARRbegin/results/coverage/GR.1h.dex.tsv")
...
res4hD = pd.read_table("/groups/lackgrp/ll_members/berkay/STARRbegin/results/coverage/GR.4h.dex.tsv")
...
res8hD = pd.read_table("/groups/lackgrp/ll_members/berkay/STARRbegin/results/coverage/GR.8h.dex.tsv")
...
res12hD = pd.read_table("/groups/lackgrp/ll_members/berkay/STARRbegin/results/coverage/GR.12h.dex.tsv")
...
dex = pd.concat([res1hD, res4hD, res8hD, res12hD]).reset_index(drop=True)
dex["comparison"] = "dex"
...
dex.loc[dex["name"].isin(bedNGR["posName"]), "nodeClass"] = "nGR"
dex.loc[dex["name"].isin(bedGR["posName"]), "nodeClass"] = "GR"
...
ind = dex.loc[(dex["log2FoldChange"] > 1) & (dex["nodeClass"] == "GR") & (dex["padj"] < 0.05) & (dex["duration"] == "8h"), ["chr", "start", "end", "posName", "nodeClass"]].sort_values(["chr", "start"])
